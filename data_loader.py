...
def load_transplant_data():
    ...
    diseases = ["nonmalignant", "ALL", "AML", "chronic", "lymphoma"]
    disease_map = {disease: i for i, disease in enumerate(diseases)}

    data["Disease"] = [disease_map[d] for d in data["Disease"]]
    data.rename(columns={"Disease": "DiseaseSeverity", "Recipientage": "RecipientAge", "Donorage": "DonorAge"}, inplace=True)
    ...
    for feature in set(data.columns) - set(["CD34kgx10d6", "survival_status"]):
